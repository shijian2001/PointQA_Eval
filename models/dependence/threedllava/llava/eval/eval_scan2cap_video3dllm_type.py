...
def calc_scan2cap_score(preds, tokenizer, scorers, args):
    ...
    for pred in preds:
        ...
        if iou >= 0.25:
            tmp_preds_iou25[key] = [f"sos {pred['text'].replace('.', ' . ').replace(',', ' , ').lower()} eos".replace('\n', ' ')]
        else:
            tmp_preds_iou25[key] = [f"sos eos"]
        if iou >= 0.5:
            tmp_preds_iou50[key] = [f"sos {pred['text'].replace('.', ' . ').replace(',', ' , ').lower()} eos".replace('\n', ' ')]
        else:
            tmp_preds_iou50[key] = [f"sos eos"]
        tmp_targets[key] = [caption.replace('.', ' . ').replace(',', ' , ').lower() for caption in gt_dict[key]]

    missing_keys = gt_dict.keys() - tmp_targets.keys()

    for missing_key in missing_keys:
        tmp_preds_iou25[missing_key] = ["sos eos"]
        tmp_preds_iou50[missing_key] = ["sos eos"]
        tmp_targets[missing_key] = [caption for caption in gt_dict[missing_key]]
