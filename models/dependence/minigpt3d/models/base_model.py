...
from ..common.utils import get_abs_path, is_url, cfg_from_yaml_file
...
class BaseModel(nn.Module):
    ...
    def init_llm(cls, llama_model_path, low_resource=False, low_res_device=0, lora_r=0,
        ...
        def _enable_layernorm(layernorm_module):
            if layernorm_module is None:
                return False
            weight = getattr(layernorm_module, "weight", None)
            if weight is None:
                return False
            weight.requires_grad = True
            weight.data = weight.data.float()
            bias = getattr(layernorm_module, "bias", None)
            if bias is not None:
        ...
        if lora_r > 0:
            ...
            for i, layer in enumerate(llama_model.model.model.layers):
                qk_layernorm = getattr(layer.self_attn, 'qk_layernorm', None)
                q_layernorm = getattr(layer.self_attn, 'q_layernorm', None)
                k_layernorm = getattr(layer.self_attn, 'k_layernorm', None)

                if not _enable_layernorm(qk_layernorm):
                    _enable_layernorm(q_layernorm)
                    _enable_layernorm(k_layernorm)

                _enable_layernorm(getattr(layer, 'post_layernorm', None))
                _enable_layernorm(getattr(layer, 'input_layernorm', None))

            final_ln = getattr(getattr(llama_model, 'model', None), 'model', None)
            if final_ln is None:
                final_ln = getattr(llama_model, 'model', None)
            if final_ln is not None:
                _enable_layernorm(getattr(final_ln, 'final_layernorm', None))
            ...
            for i, layer in enumerate(llama_model.model.layers):
                ...
                if not _enable_layernorm(qk_layernorm):
                    _enable_layernorm(getattr(layer.self_attn, 'q_layernorm', None))
                    _enable_layernorm(k_layernorm)

                _enable_layernorm(getattr(layer, 'post_layernorm', None))
                _enable_layernorm(getattr(layer, 'input_layernorm', None))

            final_ln_container = getattr(llama_model, 'model', None)
            if final_ln_container is not None:
                _enable_layernorm(getattr(final_ln_container, 'final_layernorm', None))
