...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        # load clip: default is openai tag, but can be overridden by local file path.
        clip_pretrained = os.environ.get('ONELLM_OPENCLIP_PRETRAINED', 'openai')
        clip_cache_dir = os.environ.get('ONELLM_OPENCLIP_CACHE_DIR')
        if clip_pretrained != 'openai' and not os.path.exists(clip_pretrained):
            raise FileNotFoundError(
                f"ONELLM_OPENCLIP_PRETRAINED not found: {clip_pretrained}"
            )
