...
from minigpt4.common.path_utils import resolve_weight_path
...
class MiniGPT_3D(MiniGPTBase):
    ...
    def __init__(
        ...
        self.pc_encoder = self.init_pc_encoder(pc_precision, freeze_pc, pc_encoder_ckpt_path=pc_encoder_ckpt_path)
        ...
        if qformer_pretrained_path:
            self.load_from_pretrained(url_or_filename=qformer_pretrained_path)
        else:
            logger.warning("Skipping Q-Former pretrained initialization because no local path was provided.")
    ...
    def init_Qformer(cls, num_query_token, vision_width, freeze, QFormer_lora_r,
                     train_QFormer_norm, QFormer_lora_module, bert_base_uncased_path=None):
        encoder_config = BertConfig.from_pretrained(bert_base_uncased_path or resolve_weight_path("bert-base-uncased"))
    ...
    def from_config(cls, cfg):
        ...
        pc_encoder_ckpt_path = cfg.get("pc_encoder_ckpt_path", None)
        bert_base_uncased_path = cfg.get("bert_base_uncased_path", None)
        qformer_pretrained_path = cfg.get("qformer_pretrained_path", None)
        ...
        model = cls(
            ...
            QFormer_lora_module=QFormer_lora_module,
            train_QFormer_norm=train_QFormer_norm,
            pc_encoder_ckpt_path=pc_encoder_ckpt_path,
            bert_base_uncased_path=bert_base_uncased_path,
