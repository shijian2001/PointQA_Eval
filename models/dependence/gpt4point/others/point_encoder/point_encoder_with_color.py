...
class Encoder(nn.Module):
    def __init__(self, encoder_channel, point_input_dims=6):
        super().__init__()
...
class PointTransformer_color(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        Load the weight.
        '''
        env_ckpt = os.environ.get("GPT4POINT_POINT_ENCODER_CKPT")
        if env_ckpt and os.path.isfile(env_ckpt):
            pts_checkpoint_path = env_ckpt
        elif os.path.isfile(config.checkpoint):
            pts_checkpoint_path = config.checkpoint
        else:
